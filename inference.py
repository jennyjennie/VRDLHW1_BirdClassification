...
from LoadDataSet import TestDataset
from config import *
...
# Load test data.
data_dir = {"test": "testing_images"}
image_datasets = {x: TestDataset('testing_img_order.txt', data_dir[x], data_transforms[x]) 
                 for x in ["test"]}
...
def write_pred_to_txt(file_ids, all_pred, filetag):
    ans_filename = 'answer_' + model_name + filetag
    file =  open(f"test_results/{ans_filename}.txt", "w+")
    for file_id, pred in zip(file_ids, all_pred):
        file.write(file_id + ' ' + str(int(pred)+1).zfill(3) + '.' + LABELS[int(pred)] + "\n")
    file.close()
    print("Done.")
...
def main():
    ...
    # write prediction result to txt file
    file_ids = image_datasets["test"].get_file_ids()
    write_pred_to_txt(file_ids, all_pred, args.filetag)
