...
from LoadDataSet import BirdDataSet
...
# Load dataset.
# For Testing, the dataset does not contain testing data
dataset = pd.read_csv('new_training_labels.txt', sep = ' ', names = ['id', 'label_int'])
train_info, val_info = train_test_split(dataset, test_size=0.2, stratify = dataset['label_int'])
data_dir = {'train': 'training_images', 'val' : 'training_images'}
info_file = {'train': train_info, 'val': val_info}
image_datasets = {x: BirdDataSet(info_file[x], data_dir[x], transform = data_transforms[x])
                for x in ['train', 'val']}
...
dataloaders = {}
dataloaders['train']= DataLoader(dataset = image_datasets['train'], batch_size = BATCHSIZE, shuffle = True, pin_memory  = True)
dataloaders['val']= DataLoader(dataset = image_datasets['val'], batch_size = BATCHSIZE, shuffle = True, pin_memory  = True)
#dataset_sizes = {'train': 3000, 'val': 3000} # To be modified, too ugly
...
def parse_args():
    ...
    parser.add_argument("-e", dest="num_epoch", default=25, type=int)
...
def main():
    ...
    #optimizer_ft = optim.SGD(model.fc.parameters(), lr=0.001, momentum=0.9) #DBG
    # Learning rate scheduler
    exp_lr_scheduler = lr_scheduler.CosineAnnealingLR(optimizer_ft, T_max=4)

    model_ft = train_model(model, criterion, optimizer_ft, exp_lr_scheduler,
                    checkPointStore, num_epochs=args.num_epoch)
